...
namespace collision {
    ...
    public:
        ...
        explicit HurtBox(std::vector<std::array<int,4>> bounds) : CollisionBox(bounds) {
            exist = true;
        }
    };
    ...
    protected:
    public:
        BlockBox(): CollisionBox(nullBounds) {
            exist = false;
        }

        explicit BlockBox(std::vector<std::array<int,4>> bounds) : CollisionBox(bounds) {
            exist = true;
        }
    };
